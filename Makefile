...
pdf:
	pandoc --smart --filter pandoc-citeproc --variable documentclass=templates/sig-alternate --csl=templates/acm-sig-proceedings.csl --number-sections --highlight-style=haddock -f markdown -t latex --template=templates/template.latex -o out/paper.pdf paper.md
html:
